...
(* Ex 23 and 24 *)
let _ = 
  expect "(let [x,(call0 read_bool)] (call print_bool x))" 
  @@ test_it "let val x := read_bool() in print_bool(x) end" 
  cmd_parse

let _ = 
  expect "(let [x,(call rnd 1 10)] (call print_int x))" 
  @@ test_it "let val x := rnd(1, 10) in print_int(x) end" 
  cmd_parse

let _ = 
  expect "(let [x,(call0 read_int)] (let [y,(call rnd_seed x)] (call0 print_line)))" 
  @@ test_it "let val x := read_int() val y := rnd_seed(x) in print_line() end" 
  cmd_parse

let _ = 
  expect "(let [x,(call0 read_bool)] (let [y,(call0 read_bool)] (eq x y)))" 
  @@ test_it "let val x := read_bool() val y := read_bool() in x = y end" 
  cmd_parse

let _ = 
  expect "(let [__t_1,(call0 print_int)] (call rnd (call0 print_int) __t_1))" 
  @@ test_it "rnd(print_int(), print_int())" 
  cmd_parse
...
_ti_main:
	...
	addq	$8, %rsp
	ret|q}
 @@ test_it "print_int(rnd(read_int(), read_int()))"
    cmd_gen
...
_ti_main:
	...
	addq	$8, %rsp
	ret|q}
 @@ test_it "rnd_seed(read_int())"
    cmd_gen

let _ =
  expect ""
  @@ test_it "let val x:=read_bool() in print_bool(x); print_line() end"
      cmd_run;
    expect "true\n\nDone" @@ test_command_stdin "true\n" [execf]

let _ =
  expect ""
  @@ test_it "let val x:=read_bool() val y:= read_int() in print_bool(not x) end"
      cmd_run;
    expect "true\nDone" @@ test_command_stdin "false 3\n" [execf]

let _ =
  expect ""
  @@ test_it "rnd_seed(read_int())"
      cmd_run;
    expect "Done" @@ test_command_stdin "3\n" [execf]

let _ = 
  expect_fail
  @@ test_it "let val x := read_int() in print_bool(x) end" 
      cmd_run

let _ = 
  expect_fail
  @@ test_it "print_line(5)" 
      cmd_run
