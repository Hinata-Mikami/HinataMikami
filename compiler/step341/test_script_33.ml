...
let _ =
 expect ""
 @@ test_it "print_int(1 + (read_int() + - 3))"
    cmd_run;
...
let _ =
 expect ""
 @@ test_it "let val x := read_int() in print_int(1 + x * 3) end"
    cmd_run;
  expect "301\nDone" @@ test_command_stdin "100\n" [execf]

let _ =
 expect ""
 @@ test_it "let val x := read_int() in print_int(1 + x * 3 - 1+x*2) end"
    cmd_run;
  expect "500\nDone" @@ test_command_stdin "100\n" [execf]

let _ =
 expect ""
 @@ test_it "let val x := read_int() in print_int(1 - x - x + 5) end"
    cmd_run;
  expect "-194\nDone" @@ test_command_stdin "100\n" [execf]
...
let _ =
 ...
 @@ test_it "let val x := read_int() \
            val y := \
    let val y1 := 1 + x in 2 + y1 end \
    val z :=4+y in print_int(10+x+(y+z)) end"
    cmd_run;
  expect "320\nDone" @@ test_command_stdin "100\n" [execf]

let _ =
 expect ""
 @@ test_it "print_int(let val x := read_int() \
        val y := \
    let val y1 := 1 + x in 2 + y1 end \
    val z := y in 10+x+(y+z) end)"
    cmd_run;
