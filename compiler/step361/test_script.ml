...
(* Ex 31 *)
let _ = 
  expect "(if (eq x 1) 1 (if (eq y 1) 1 3))" 
  @@ test_it "if x = 1 then 1 elif y = 1 then 1 else 3 fi"
  cmd_parse

let _ = 
  expect "(if (eq x 1) 1 (if (eq y 2) 2 (if (eq z 3) 3)))" 
  @@ test_it "if x = 1 then 1 elif y = 2 then 2 elif z = 3 then 3 fi"
  cmd_parse

let _ = 
  expect_fail
  @@ test_it "if x = 1 then 1 elif fi"
  cmd_parse

let _ = 
  expect_fail
  @@ test_it "if x = 1 then 1 elif y = 1 fi"
  cmd_parse
...
let _ =
 ...
 @@ test_it {q|
  ...
  let val x := read_int()
  in
	if x = 1 then 1 elif x = 2 then 2 else 0 fi
  end)
 |q}
    cmd_run;
  expect "1\nDone" @@ 
...
let _ =
 ...
 @@ test_it {q|
  ...
  let val x := read_int()
  in	
	if x = 1 then false elif x = 1 then false elif x = 0 then true else false fi
  end)
 |q}
    cmd_run;
  expect "true\nDone" @@ 
