...
%token IF THEN ELSE ELIF FI WHILE DO DONE FOR TO
...
%nonassoc TRUE FALSE INT IDENT LPAREN STRING IDENTPAREN IF WHILE DO DONE FOR TO
...
exp_complex:
 ...
 | IF exp THEN exp rest_if { match $5 with 
   | None   -> M.if2 $2 $4
   | Some e -> M.if3 $2 $4 e }
 | WHILE exp DO exp DONE { M.while_loop $2 $4 }
 | FOR IDENT COLONEQUAL exp TO exp DO exp DONE
    {
      M.local ~mut:true ($2, $4) 
        (M.while_loop 
          (M.geq  $6 (M.var $2) )
          (M.seq $8 (M.assign $2 (M.add (M.atomic (M.var $2)) (M.int 1L)))))
    }
;
