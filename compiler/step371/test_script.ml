...
let _ = 
  expect_fail 
  @@ test_it "while true do done"
  cmd_parse

let _ = 
  expect_fail 
  @@ test_it "while true do x := x + 1"
  cmd_parse

let _ = 
  expect_fail 
  @@ test_it "let val x := true in while do x := not x done"
  cmd_parse

let _ = 
  expect "(let [x,1] (while (lt x 10) do (assign x (add x 1))))" 
  @@ test_it "let val x := 1 in while x < 10 do x := x + 1 done end"
  cmd_parse

let _ = 
  expect_fail
  @@ test_it "let var x := true in while true do x := x + 1 done end" 
  cmd_gen

let _ = 
  expect_fail
  @@ test_it "let var x := 0 in while x < 3 do x := true done end" 
  cmd_gen
...
.L_2:
	addq	$8, %rsp
	ret|q}
 @@ test_it "let var x := 1 in while x <= 3 do print_int(x); print_line(); x := x+1 done end"
    cmd_gen
...
let _ = 
  expect "" @@ test_it {q| 
	let var x := 0 in 
  while x < 3 do x := x + 1; print_int(x); print_line() done end
  |q}
  cmd_run;
	expect "1\n2\n3\n\nDone" @@ 
...
let _ =
	expect ""@@ test_it {q|
	let var x := read_bool() in ((while x <> true do x := not x done); print_bool(x)) end
		|q}
			 cmd_run;
		 expect "true\nDone" @@ 
		 test_command_stdin "false\n" [execf]


(* Ex 36 *)
let _ = 
  expect_fail 
  @@ test_it "for 1 do 1 + 1 done"
  cmd_parse

let _ = 
  expect_fail 
  @@ test_it "for true do x := x + 1 done"
  cmd_parse

let _ = 
  expect "(let [*i,1] (while (geq 3 i) do (seq (assign x (add x 1)) (assign i (add i 1)))))" 
  @@ test_it "for i := 1 to 3 do x := x + 1 done"
  cmd_parse

let _ = 
  expect_fail
  @@ test_it "for i := 1 to 3 do i := true done" 
  cmd_gen
...
.L_2:
	...
	addq	$24, %rsp
	ret|q}
 @@ test_it "	let var x := read_int() in for i := 1 to 3 do x := x + 1 done; print_int(x) end"
    cmd_gen


let _ =
	expect ""@@ test_it {q|
	let var x := read_int() in
	for i := 1 to 3 do x := x + 1 done; print_int(x) end
		|q}
			 cmd_run;
		 expect "3\nDone" @@ 
		 test_command_stdin "0\n" [execf]
...
let _ =
	expect ""@@ test_it {q|
	let var x := 0 in for i := 1 to 3 do x := x + i done; print_int(x) end
		|q}
			 cmd_run;
		 expect "6\nDone" @@ 
		 test_command_stdin "0\n" [execf]
